...
export class UserProblemsService {
  ...
  async updateCodeForProblem(
    problem: Problem,
    code: string,
    userId?: string,
  ): Promise<{ userProblem: UserProblem; created: boolean }> {
    const where: UserProblemWhere = { problem: { id: problem.id } };
    if (userId) {
    ...
    if (!userProblem) {
      userProblem = this.userProblemRepo.create({ problem });
      if (userId) {
  ...
  async linkProblemToUser(
    ...
    const where: UserProblemWhere = { problem: { id: problemId } };
    if (userId) {
      where.user = { id: userId };
    } else {
      where.user = null;
